...
/*
          ...
          [
            ...
            },
            {
              ...
              onPress: async () => {
                const formData = new FormData();
                formData.append('idDetalle', idDetalle);
                const response = await fetch(`${ip}servicios/cliente/pedido.php?action=deleteDetail`, {
                  method: 'POST',
                  body: formData
                });
                const data = await response.json();
                if (data.status) {
                  Alert.alert('Datos eliminados correctamente del carrito');
                  // Llamar a la función de actualización para actualizar la lista
                  updateDataDetalleCarrito(prevData => prevData.filter(item => item.id_detalles_pedidos !== idDetalle));
                } else {
                  Alert.alert('Error al eliminar del carrito', data.error);
                }
              }
  ...
  return (
    ...
    <Text style={styles.itemText}>ID: {item.id_detalles_pedidos}</Text>
    <Text style={styles.itemText}>Nombre: {item.nombre_libro}</Text>
    <Text style={styles.itemText}>Precio: ${item.precio_producto}</Text>
    <Text style={styles.itemText}>Cantidad: {item.cantidad_comprada}</Text>
    <Text style={styles.itemText}>SubTotal: ${(parseFloat(item.cantidad_comprada)*parseFloat(item.precio_producto)).toFixed(2)}</Text>

    <TouchableOpacity style={styles.modifyButton}
    onPress={()=>accionBotonDetalle(item.id_detalles_pedidos, item.cantidad_comprada)}
    >
